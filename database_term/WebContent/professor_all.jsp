...
<body>
	<table class="table table-hover">
		...
		<%
			...
			try {
				while (result != null && result.next()) {
					int count = 0;
					String s = "select COUNT(*) as count from enroll where c_id='" + result.getString("id")
							+ "' AND c_id_no=" + result.getInt("id_no");
		...
		<tr>
			...
			<td><a
				href="course_delete.jsp?c_id=<%=result.getString("id")%>&c_id_no=<%=result.getInt("id_no")%>">삭제</a></td>
			<td><a
				href="course_update.jsp?c_id=<%=result.getString("id")%>&c_id_no=<%=result.getInt("id_no")%>">수정</a></td>
		</tr>
		<%
			}
